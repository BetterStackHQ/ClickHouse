...
protected:
    ...
    /// `StorageMergeTree::loadMutations` does not have to list it again. Written once by
    /// `loadDataParts` and taken once by `loadMutations`, both from the table's constructor before
    /// it is published, so no synchronisation is needed. `loadDataParts` drops them again for a
    /// table that has no `loadMutations` to take them, so they are never kept for the life of the
    /// table.
    MutationEntryNamesByDisk mutation_entries_seen_by_parts_scan;

    /// Empty until the parts scan has run, and emptied by this call, so a second consumer would
    /// list the directory itself rather than silently see nothing.
