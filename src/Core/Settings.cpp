...
When reading an object from S3 (or an S3-compatible store such as GCS), check that every GET request returns the same ETag that was observed when the object was listed. A single file read issues many ranged GET requests; if the object is overwritten in place between them (for example by an external writer rewriting a fixed key), the reads can otherwise be stitched together from two different object generations and surface as a corrupted checksum or parse error. When a mismatch is detected the read fails with `S3_OBJECT_CHANGED_DURING_READ` instead of returning inconsistent data. Disable only for workloads that intentionally read objects that are being overwritten and can tolerate inconsistent reads.
    ...
    DECLARE(Bool, use_object_metadata_cache, false, R"(
...
Enable only when the objects read this way are immutable, that is, never overwritten in place. Cached entries are never revalidated, so if an object is overwritten anyway, a query can still return the previous content: reads served from the filesystem cache, and the row count and schema inference caches (which validate against the cached last modification time), never reach the object store and so cannot observe the overwrite. The absence of an object is never cached. The cache is bounded by the `object_metadata_cache_max_entries` server configuration parameter (default 100000).

A read that does reach S3 checks the ETag and total size of every response against the cached entry, independently of `s3_validate_etag_on_read`, which only decides whether the read is additionally pinned with `If-Match`. An object found to have been overwritten before the read emitted anything is read again under its current metadata, counted by the `ObjectMetadataCacheRewriteRecovered` profile event and reported as a warning in the server log - the recovery does not make the broken guarantee any less worth investigating. Once rows have been emitted the read fails instead, because resuming or restarting it would mix two generations of the object. Azure and HDFS reads have no equivalent check.
...
Use async and potentially multithreaded execution of materialized view query, can speedup views processing during INSERT, but also consume more memory.)", 0) \
    DECLARE(Bool, use_materialized_view_select_plan_cache, false, R"(
Cache the analyzed and optimized select plan of a materialized view across inserted blocks and inserts, instead of re-planning the view query for every pushed block. The cache is invalidated when the view query, the metadata of the involved tables, SQL user defined functions, or row policies change. View selects that would freeze analysis-time state into the plan (such as `now` over constant arguments, subqueries, or reads from other tables) are never cached.
...
- 1 — Enabled.
    ...
    DECLARE(UInt64, materialized_view_select_plan_cache_max_variants, 16, R"(
Maximum number of select plan variants (distinct combinations of input block structure, settings, user and roles, and analyzer choice) cached per materialized view when [use_materialized_view_select_plan_cache](#use_materialized_view_select_plan_cache) is enabled. The oldest variant is replaced beyond the limit.
)", 0) \
