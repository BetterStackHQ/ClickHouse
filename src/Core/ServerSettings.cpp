...
:::
    ...
    DECLARE(UInt64, max_named_collections_loading_thread_pool_size, 16, R"(The number of threads to read and parse SQL-defined named collections at startup. A value of `0` or `1` loads them sequentially in the calling thread. Only applies to the `local` storage of named collections; with Keeper storage they are always loaded sequentially.)", 0) \
    DECLARE(UInt64, max_access_entities_recovery_thread_pool_size, 16, R"(The number of threads to read and parse the files of the local access storage when its `.list` files have to be rebuilt at startup. A value of `0` or `1` reads them sequentially in the calling thread. Has no effect on a startup that finds usable `.list` files, which reads the entity files lazily.)", 0) \
